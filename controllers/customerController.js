...
exports.updateCustomer = async (req, res) => {
  try {
    ...
    const updatedCustomer = await Customer.findOneAndUpdate(
      { _id: customerId },
      { $set: { name: newData.name, email: newData.email, mobileNo: newData.mobileNo, address: newData.address } },
      { new: true }
    );
...
exports.updateDueAmt = async (req, res) => {
  try {
    ...
    const customer = await Customer.findByIdAndUpdate(customerId, { dueAmt: newDueAmt }, { new: true });
    if (customer) {
      console.log('DueAmt updated successfully:', customer);
      res.status(200).json({success:true, message: 'DueAmt updated successfully', customer });
    } else {
      console.log('Customer not found');
      res.status(404).json({ error: 'Customer not found' });
    }
